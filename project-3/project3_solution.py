...
class Classifier:
  ...
  def fit(self, X, Y):
    ...
    # TASK 1:
    # TODO: get all unique label values, and store them in self.labels
    self.labels = np.unique(Y) # Fill this in
    ...
    # TASK 2:
    # TODO: Normalize the input data, so every value is from 0 to 1
    ...
    for i in range(n):
      ...
      squared = row ** 2 # TODO: Fill this in by squaring the current data row
      # 2: TODO: running average: we have to add this new squared row to the positive coef array
      # use the cumulative average formula to combine the values
      pos = ((pos * i) + squared) / (i + 1)
      ...
      inverted = 1 - row # TODO: Fill this in to invert the values in the current row
      # 2: square the new values, just like before
      squared = inverted ** 2 # TODO: Fill this in
      # 3: TODO: update the negative coef array, again using cumulative average
      # this should be practically the same code as you wrote above, just replacing 'pos' with 'neg'
      neg = ((neg * i) + squared) / (i + 1)
  ...
  def predict(self, X):
    ...
    def label(row):
      ...
      for label in self.labels:
        ...
        scores[label] = np.sum((row * pos) - (row * neg)) # TODO: fill this in
    ...
    return np.apply_along_axis(label, 1, X) # TODO: fill this in with a line that applies the 'label' function to each row
  
# STEP 3: Prepare the model
# TASK 1: split the data
# This should be easy at this point: use train test split to split 'data' and 'target',
# 80% for training, 20% for testing
train_X, test_X, train_Y, test_Y = train_test_split(data, target, test_size=0.2) # TODO: fill this in
...
c = Classifier() # TODO: fill this in
# 2: Call the fit method, passing in the training data
c.fit(train_X, train_Y) # TODO: fill this in

# TASK 3: Evaluating the model
# subtasks:
# 1: Call the predict method on your model, passing in the test X data as input.
predicted_Y = c.predict(test_X) # TODO: fill this in
# 2: compare how many labels in the predicted results match the expected results (test_Y)
# Print out the accuracy, which is (number of matches) / (total number of test data points)
# TODO: calculate and print the accuracy
matches = predicted_Y == test_Y
